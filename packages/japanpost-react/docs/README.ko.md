...
- 검증 기준선: Chrome {{MIN_CHROME}}입니다.
- 계약의 정본은 한 곳입니다. 이 패키지 `package.json`의 `browserslist` 필드이며
  현재 값은 `{{BROWSERSLIST_QUERY}}`입니다. 빌드 타깃, 두 호환성 게이트, 이 절이
  모두 여기서 파생됩니다.
- `dist/*.es.js`는 Chrome {{MIN_CHROME}}이 파싱하는 문법으로 배포되며 빌드마다
  검사합니다.
- 필요한 호스트 전역: `fetch`, `AbortController`, `globalThis`, `Promise`,
  `setTimeout`, `clearTimeout`. 모두 Chrome {{MIN_CHROME}}에 네이티브로
...
- Safari 13.1, Firefox 74 같은 동시대 브라우저도 산출물을 파싱할 수 있으나
  검증 대상은 Chrome {{MIN_CHROME}}뿐입니다.
- React/React DOM 자체의 브라우저 호환성은 소비자 책임입니다. 참고로
  `react-dom@19.2.8` 프로덕션 번들에는 Chrome {{MIN_CHROME}}을 초과하는 문법과
  런타임 API가 없습니다.
