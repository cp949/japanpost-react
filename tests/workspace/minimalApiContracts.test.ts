...
describe("minimal api shared contracts", () => {
  it("keeps adapter request and response types aligned with the package contract", () => {
    expectTypeOf<
      Parameters<AddressAdapter["searchcode"]>[0]
    >().toEqualTypeOf<JapanPostSearchcodeRequest>();
    expectTypeOf<
      Parameters<AddressAdapter["addresszip"]>[0]
    >().toEqualTypeOf<JapanPostAddresszipRequest>();
    expectTypeOf<
