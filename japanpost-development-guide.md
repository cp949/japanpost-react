...
우편번호로 주소를 조회한다. `code`는 7자리 숫자 문자열이어야 한다.
...
| ----------- | ------------------------------------ | -------------------------------------------------------------- |
| `400`       | 우편번호 형식 오류 (7자리 숫자 아님) | `{ "error": "Postal code must contain exactly 7 digits" }`     |
...
```http
GET https://api.da.pf.japanpost.jp/api/v2/searchcode/{zip_code}
