...
중요: reference minimal-api가 pager-aware high-level 계약으로 정렬되면서,
`packages/japanpost-react`의 공개 계약도 같은 구조로 맞춰졌다.
...
우편번호로 주소를 조회한다. 요청 body는 공개 타입과 동일한
`value`, `pageNumber`, `rowsPerPage`, `includeParenthesesTown`를 받는다.
...
주소를 검색한다. 요청 body는 공개 타입과 동일한 camelCase 필드와
...
| `200`       | addresszip 검색 필드가 모두 비어 있음 | empty page payload                                             |
| `404`       | reference adapter가 upstream miss를 오류로 보존한 경우 | `{ "error": "No matching addresses found" }`                   |
...
- 즉 upstream 전체 스펙 중에서, 현재 저장소는 public camelCase body를 받아
  upstream snake_case body로 변환한다.
- upstream 응답의 `addresses[]`는 adapter에서 정규화된 `JapanAddress`의
  `elements[]`로 매핑된다.
