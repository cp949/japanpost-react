...
배포 라이브러리의 브라우저 기준선은 `packages/japanpost-react/package.json`의
`browserslist` 필드 한 곳에 선언합니다. 빌드 타깃, 두 호환성 게이트, 패키지
README가 모두 이 값에서 파생되며 현재 값은
[브라우저 지원](./packages/japanpost-react/README.ko.md#브라우저-지원) 절에서
확인합니다. 데모 앱(`apps/demo`)은 이 기준선의 대상이 아닙니다.
`@mui/material@9`의 문서상 최소 지원 버전이 Chrome 117이기 때문입니다.
