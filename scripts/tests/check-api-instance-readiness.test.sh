...
if [[ "$url" == *"/health" ]]; then
  printf '%s' '{"ok":true,"instanceId":"wrong-instance"}'
  exit 0
fi
...
(
  cd "$TMP_DIR"
  timeout 3 bash "$ROOT_DIR/scripts/check-api.sh" >"$TMP_DIR/stdout.log" 2>"$TMP_DIR/stderr.log"
)
status=$?
set -e

if [[ "$status" -eq 0 ]]; then
  echo "check-api.sh unexpectedly succeeded with a mismatched instance id" >&2
  exit 1
fi
